...
import { manageCategoryServices } from "../../services/ManageCategoryServices";
...
export const AddCateAction = (data) => {
    return async dispatch => {
        try {
            const result = await manageCategoryServices.addCate(data);
...
export const GetAllCateAction = () => {
    return async dispatch => {
        try {
            const result = await manageCategoryServices.getAllCate();
...
export const GetDetailCateAction = (id) => {
    return async dispatch => {
        try {
            const result = await manageCategoryServices.getDetailCate(id);
            if (result.status === 200) {
                dispatch({
...
export const UpdateCateAction = (id, data) => {
    return async dispatch => {
        try {
            const result = await manageCategoryServices.updateCate(id, data);
            if (result.status === 200) {
...
export const DeleteCate = (id) => {
    return async dispatch => {
        try {
            const result = await manageCategoryServices.delCate(id);
            if (result.status === 200) {
